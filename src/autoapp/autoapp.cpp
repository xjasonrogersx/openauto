...
std::optional<boost::log::trivial::severity_level> parseLogLevel(
...
std::optional<boost::log::trivial::severity_level> parseLogLevelFromArgs(
    int argc, char* argv[]) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--log_level") {
      ...
      OPENAUTO_LOG(warning)
          << "[OpenAuto] --log_level provided without a value.";
      return std::nullopt;
    }
...
int main(int argc, char* argv[]) {
  ...
  mainWindow.move(std::max(0, (width - windowedWidth) / 2),
                  std::max(0, (height - windowedHeight) / 2));
  mainWindow.show();
  ...
  autoapp::mqtt::NightModeStateSubscriber nightModeSubscriber(
      [&mainWindow](bool active) {
  ...
  autoapp::mqtt::MediaPlayerCommandSubscriber mediaPlayerCommandSubscriber(
      [&app](const std::string& command) {
        if (command == "play") {
          app->sendAndroidMediaButton(aap_protobuf::service::media::sink::
                                          message::KeyCode::KEYCODE_MEDIA_PLAY);
          return;
        }

        if (command == "stop") {
          app->sendAndroidMediaButton(
        ...
        if (command == "next") {
          app->sendAndroidMediaButton(aap_protobuf::service::media::sink::
                                          message::KeyCode::KEYCODE_MEDIA_NEXT);
          return;
        }
        ...
        OPENAUTO_LOG(warning)
            << "[AutoApp] Unknown media command received: " << command;
      });
  ...
  app->setConnectionStateHandler([&mainWindow, width, height](bool connected) {
    ...
    QMetaObject::invokeMethod(
        &mainWindow,
        [&mainWindow, width, height, connected]() {
          if (connected) {
            mainWindow.showFullScreen();
            OPENAUTO_LOG(info) << "[AutoApp] Switched main window to "
                                  "fullscreen after connection.";
          } else {
            const int windowedWidth =
                std::min(width, std::max(800, (width * 9) / 10));
            const int windowedHeight =
                std::min(height, std::max(480, (height * 9) / 10));
            mainWindow.showNormal();
            mainWindow.resize(windowedWidth, windowedHeight);
            mainWindow.move(std::max(0, (width - windowedWidth) / 2),
                            std::max(0, (height - windowedHeight) / 2));
            OPENAUTO_LOG(info) << "[AutoApp] Restored main window to normal "
